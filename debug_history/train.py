...
base_lr = 1e-4  
pth_load_path = './log/snapshot/1024train_Vimeo_2_80epoch/ckpt_best_79.pth'#_Vimeo_all_singleGPU/best_19.model' # ckpt_best_%s.pth' % (str(start_epoch))#/trainOnlyF2I.model'
save_path = './log/snapshot/'
# path_log = save_path+name+"/train.log"#FILE_WRITE = open(path_log,"a")  #只追加到文件末尾
# 不再使用 path_log ,因为cat 命令 会导致这行代码出bug
...
def train(epoch, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        if (batch_idx % print_step) == 0:
            ...
            log5 = 'Ms-ssim of image: {:.6f}'.format(msssim_image)
            log6 = 'Ms-ssim of feature: {:.6f}'.format(msssim_fea)
            print(time_now+"\n"+log0+"\n"+log1+"\n"+log2+"\n"+log3+"\n"+log4+"\n"+log5+"\n"+log6+"\n")
