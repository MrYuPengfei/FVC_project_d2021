...
class DataSet_ClassB(data.Dataset):
    def __init__(self, root="/data1/Data/H265/AI_22_hm",
                 # filelist="/data1/Data/HEVC_dataset_original/ls.txt",
                 filelist="./filelists/B_pengfei.txt",
                 # refdir='H265L20', testfull=True):# 高寒使用的是HM16.20生成的I帧?
                 refdir='H265L22',testfull=True):          # 鹏飞使用的是无损的I帧？
        ...
        for folder in folders:
            seq = folder.rstrip()
            seqIbpp = AllIbpp[0]
            # seqIbpp = AllIbpp[ii]     # 逻辑上讲，每个视频的ibpp应该不一样
            imlist = os.listdir(os.path.join(os.path.join(root,refdir),seq))
            ...
            if refdir == 'original':
                for i in range(framerange):
                    refpath = os.path.join(root, str(seq)  + '/im' + str(i * 10 + 1).zfill(3) + '.png')
                    inputpath = []
                    for j in range(10):
                ...
                for i in range(framerange):
                    refpath = os.path.join(root, str(refdir) + '/' + str(seq) + '/im' + str(i * 10 + 1).zfill(3) + '.png')
                    inputpath = []
                    for j in range(8):
                        inputpath.append(os.path.join(root,
                                                      str(refdir) + '/' + str(seq) + '/im' + str(i * 10 + j + 2).zfill(
                                                          3) + '.png'))
                    inputpath.append(os.path.join(root,
                                                  str(refdir) + '/' + str(seq) + '/im' + str((i+1) * 10).zfill(
                                                      3) + '.png'))
                    self.ref.append(refpath)
                    self.refbpp.append(seqIbpp)
                    self.input.append(inputpath)
    ...
    def getbpp(self, ref_i_folder):
        ...
        elif ref_i_folder == 'H265L22': # added at 20210825 pengfei
            print('use H265Q22')
            Ibpp = [1.07225491, 0.660947184, 0.850405518, 0.508370633, 0.949806552] # will not cause error, but this data was not real!
        elif ref_i_folder == 'original':
            print('use original')
            Ibpp = [1.07225491, 0.660947184, 0.850405518, 0.508370633, 0.949806552] # will not cause error, but this data was not real!
        else:
            print('cannot find ref : ', ref_i_folder)
...
class DataSet_ClassC(data.Dataset):
    def __init__(self, root="/data1/Data/H265/AI_22_hm",
                 # filelist="/data1/Data/HEVC_dataset_original/ls.txt",
                 filelist="./filelists/C_pengfei.txt",
                 # refdir='H265L20', testfull=True):# 高寒使用的是HM16.20生成的I帧?
                 refdir='H265L22',testfull=True):          # 鹏飞使用的是无损的I帧？
                 # os.system(r"ls " + root + r"/*yuv >" + filelist)    # 删除了这个用法，因为该方法导致文件名称里包含了路径
        # filelist 是手工导入的。在linux上使用 ls *yuv >ls.txt
...
class DataSet_ClassD(data.Dataset):
    def __init__(self, root="/data1/Data/H265/AI_22_hm",
                 # filelist="/data1/Data/HEVC_dataset_original/ls.txt",
                 filelist="./filelists/D_pengfei.txt",
                 # refdir='H265L20', testfull=True):# 高寒使用的是HM16.20生成的I帧?
                 refdir='H265L22',testfull=True):          # 鹏飞使用的是无损的I帧？
                 # os.system(r"ls " + root + r"/*yuv >" + filelist)    # 删除了这个用法，因为该方法导致文件名称里包含了路径
        # filelist 是手工导入的。在linux上使用 ls *yuv >ls.txt
...
class HEVCDataSet(data.Dataset):
    def __init__(self, root="/data1/Data/H265/AI_22_hm",
                 # filelist="/data1/Data/HEVC_dataset_original/ls.txt",
                 filelist="/data1/Data/H265/AI_22_hm/AI_22_hm.txt",
                 # refdir='H265L20', testfull=True):# 高寒使用的是HM16.20生成的I帧?
                 refdir='H265L22',testfull=True):          # 鹏飞使用的是无损的I帧？
                 # os.system(r"ls " + root + r"/*yuv >" + filelist)    # 删除了这个用法，因为该方法导致文件名称里包含了路径
        # filelist 是手工导入的。在linux上使用 ls *yuv >ls.txt
...
class DataSet(data.Dataset):
    ...
    def get_vimeo(self, rootdir="/data1/Data/vimeo_septuplet/sequences/",
        ...
        for n, line in enumerate(data, 1):
            ...
            refnumber_p = int(y[-6]) - 1
            refname_p = y[0:-5] + str(refnumber_p) + '.png'
            fns_train_input_p += [refname_p]
        # print(fns_train_input_b, fns_train_input_p, fns_train_ref)
    ...
    def __getitem__(self, index):
        print(self.image_input_b_list[index])
...
class DataSet_ClassB_v1(data.Dataset):
...
class Vimeo_all(data.Dataset):
    def __init__(self, path="./filelists/video-90k.txt"):
