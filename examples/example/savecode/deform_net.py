...
# from subnet.analysis_mv import Analysis_mv_net1
# from subnet.analysis import Analysis_net1
# from subnet.synthesis_mv import Synthesis_mv_net1
# from subnet.synthesis import Synthesis_net1
...
class Offset_Generator(nn.Module):
    def __init__(self):
        ...
        # self.conv_first = nn.Conv2d(3, 64, 5, stride=2)
        # self.conv_first1 = nn.Conv2d(6, 128, 3, stride=1, padding=1)
        self.conv_first2 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=1)
        # self.conv_first3 = nn.Conv2d(9, 192, 3, stride=1, padding=1)
        # self.residual_layer1 = self.make_layer(Res_Block1, 3)
        self.residual_layer2 = self.make_layer(Res_Block2, 3)
        # self.residual_layer3 = self.make_layer(Res_Block3, 3)
        self.relu = nn.ReLU(inplace=True)
        # deformable
    ...
    def generator(self, input, ref):
        ...
        # res = self.conv_first2(input - ref)
...
class Compensation(nn.Module):
    def __init__(self):
        ...
        self.dconv = DeformConv(64, 64, (3, 3), padding=1, deformable_groups=8)
        # xavier initialization
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
