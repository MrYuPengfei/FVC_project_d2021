...
bpp_ideal = [0.172821, 0.172821, 0.172821, 0.172821]
psnr_ideal = [33.221489, 33.221489, 33.221489, 33.221489]
msssim_ideal = [0.972584, 0.972584, 0.972584, 0.972584]


def plt_ClassC(lbpp=bpp_ideal, lpsnr=psnr_ideal, lmsssim=msssim_ideal, global_step=0, la='new', testfull=False):
    prefix = 'performance'
    if testfull:
    ...
    savepathpsnr = prefix + '/ClassC_psnr' + '.png'
    print(prefix)
...
#     # Ours very fast
    ...
    savepathmsssim = prefix + '/' + 'ClassC_msssim' + '.png'
    plt.legend(handles=[h264, h265, baseline, test], loc=4)
    plt.grid()
    plt.xlabel('Bpp')
    plt.ylabel('MS-SSIM')
    plt.title('ClassC_msssim')
    plt.savefig(savepathmsssim)
    plt.clf()


if __name__ == '__main__':
    plt_ClassC(la=labelname, testfull=True)