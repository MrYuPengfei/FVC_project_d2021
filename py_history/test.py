...
if __name__ == "__main__":
    ...
    # model = torch.load("./snapshot/ckpt_best_0.pth") # 20210824 发生了错误
    # model = models.squeezenet1_1(pretrained=True)
    # model = torchvision.model.load_state_dict(torch.load(pthfile))
    # print(net)
    # print(model)
    # print(type(model))
    # exit()
    # net = torchvision.model.cuda()
    # net = torch.nn.DataParallel(net.cuda(), device_ids=[0])
    pthfile = r'./snapshot/ckpt_best_0.pth'
    # model = torch.load(pthfile)   # error
    # net = torch.nn(model)         # error
    # model = torch.load("/home/gaohan/FVC/Deformable_1.model")
    model = torch.load(pthfile)
    net = model.cuda()
    net = torch.nn.DataParallel(net.cuda(), device_ids=[0])
    
    # net = torch.nn.DataParallel(model) # happend error at 20210825.
    data_transform = transforms.Compose([transforms.ToTensor()])
    global train_dataset, test_dataset
    test_dataset = HEVCDataSet()
    print('testing HEVC Class D...')
